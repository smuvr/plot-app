...
const stats = [
  ...
  {value: '100', suffix: '%', label: 'KONTROLA JAKOŚCI'}, // Изменено на число для демонстрации
  {value: 'ISO', label: 'CERTYFIKOWANE'},
  {value: '24', suffix: '/7', label: 'WSPARCIE'},
];
...
const FeatureIcon: React.FC<{ icon: React.ElementType<IconProps> }> = ({icon: Icon}) => (
  <div
    className="flex-shrink-0 w-16 h-16 bg-orange-500/10 border border-orange-500/20 rounded-full flex items-center justify-center mb-6">
    <motion.div
            ...
            return (
              ...
              >
                <p className="text-4xl sm:text-5xl font-bold text-orange-500 mb-2">
                  {isNumeric ? <Counter to={parseFloat(stat.value)}/> : stat.value}{stat?.suffix ?? ''}
                </p>
